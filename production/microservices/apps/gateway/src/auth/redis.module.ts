...
import { hosts } from '@_shared/consts/hosts';

export const REDIS_CLIENT = 'REDIS_CLIENT';
export const RedisClientProvider: Provider = {
  provide: REDIS_CLIENT,
  useFactory: (
    cacheManager: Cache & { store: { client: RedisClientConnectionType } },
  ) => {
  ...
  imports: [
    CacheModule.registerAsync({
      ...
      useFactory: async (configService: ConfigType<typeof redisConfig>) => {
        const redisURI = `redis://:${configService.password}@${hosts.redis_auth}:6379`; // Replace with your Redis URI
