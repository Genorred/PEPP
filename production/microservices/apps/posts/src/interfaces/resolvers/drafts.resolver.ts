...
export class DraftsResolver {
  constructor(
    private readonly draftsService: DraftsUseCase,
  ...
  @Mutation(() => Post)
  publishDraft(
    @Args('publishDraftInput') publishDraftInput: UpdateDraftInput,
    @CurrentUser() user: CurrentUserI,
  ): Promise<Post> {
    return this.draftsService.publishDraft({
      ...publishDraftInput,
      userId: user?.sub,
    });
  }
  ...
  findOne(
    @Args('findDraftInput') findOneInput: FindDraftInput,
    @CurrentUser() user: CurrentUserI,
  ): Promise<Draft> {
  ...
  updateDraft(
    ...
    return this.draftsRepository.update({
      ...updateDraftInput,
      userId: user?.sub,
    });
  }
  ...
  @Mutation(() => Draft)
  removeDraft(
    @Args('removeDraftInput') removeInput: RemovePostInput,
    @CurrentUser() user: CurrentUserI,
  ): Promise<Draft> {
