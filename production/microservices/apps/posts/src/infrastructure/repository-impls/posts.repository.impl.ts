...
export class PostsRepositoryImpl implements PostsRepository {
  ...
  count(input: FindManyDto): Promise<number> {
    return this.prismaService.post.count(this._findPostsParams(input));
  }
    ...
    return {
      where: {
        id: ids
          ...
          : id,
        AND:
          topicsOrSubTopics?.length &&
          topicsOrSubTopics.map((title) => ({
            OR: [
              {
                topics: {
                  some: {
  ...
  async getCommentsQuantity(
    countComments: CountCommentsByPostDto,
  ): Promise<number> {
