...
const getUserPrefKey = {
  ...
  recentlyShowedList: (userId: number) =>
    userPrefKeys.hash.recentlyShowedList + userId,
  recentlyShowedSuspended: (userId: number) =>
    userPrefKeys.list.recentlyShowedSuspended + userId,
};
...
export class PreferencesRepositoryImpl implements PreferencesRepository {
  ...
  public async get(userId: number, isFirstPage: boolean) {
    ...
    return {
      ...
      pressedPosts,
      recentlyShowedPosts: [
        ...Object.entries(recentlyShowed),
        ...recentlyShowedSuspended.map((postId) => [postId, '1']),
      ] as [string, string][],
    };
  }

  public setRecentlyShowed(
    userId: number,
    posts: {
      id: string;
    }[],
    ...
    if (posts.length > 0) {
      return this.redisClient.lPush(
        getUserPrefKey.recentlyShowedSuspended(userId),
  ...
  private async applySuspendedPosts(
    keys: UserPrefKeys,
    ...
    const length =
      Object.values(recentlyShowed).length + recentlyShowedSuspended.length;
    const difference = Math.max(length - maxLength, 0);
    if (difference) {
      // Удаляем самый старый ключ из списка
      const list = await this.redisClient.lRange(
        keys.recentlyShowedList,
        0,
        difference,
      );
