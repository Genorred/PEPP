...
describe('Posts use case', () => {
  ...
  it('should recommend items and take whole data from db', async () => {
    ...
    searchService.search.mockResolvedValue({
      totalPages: 0,
      data,
    } as unknown as SearchDtoResponse);

    expect(await postsUseCase.recommendations({})).toStrictEqual({
      totalPages: 0,
      data: dbData,
    });
  ...
  it('should recommend items depending on user', async () => {
    ...
    expect(preferencesRepositoryMock.setRecentlyShowed).toHaveBeenCalledWith(
      params.userId,
      data,
    );
    expect(searchService.search).toHaveBeenCalledWith({
    ...
    expect(preferencesRepositoryMock.get).toHaveBeenCalledWith(
      params.userId,
      true,
    );
  });
