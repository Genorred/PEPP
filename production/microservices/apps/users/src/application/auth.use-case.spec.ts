...
describe('AuthUseCase', () => {
  ...
  describe('register', () => {
    ...
    it('конфликт email', async () => {
      ...
      await expect(
        service.register({ email: 'a', username: 'b' } as any),
  ...
  describe('confirmUserEmail', () => {
    it('успешное подтверждение', async () => {
      ...
      await expect(
        service.confirmUserEmail('token', context),
    ...
    it('ошибка создания пользователя', async () => {
      ...
      const context = { res: {} } as any;
      await expect(service.confirmUserEmail('token', context)).rejects.toThrow(
        'error creating user',
      );
    });
  });
  ...
  describe('login', () => {
    it('успешный вход', async () => {
      ...
      ).resolves.toMatchObject({ id: 1, email: 'a@a.com', username: 'user' });
      expect(tokenService.setTokens).toBeCalled();
    });
  ...
  describe('logout', () => {
    it('успешный выход', async () => {
      ...
      jest
        .spyOn(require('@_shared/utils/getCookies'), 'default')
        .mockReturnValue({ refreshToken: 'token' });
    ...
    it('нет refreshToken', async () => {
      ...
      jest
        .spyOn(require('@_shared/utils/getCookies'), 'default')
        .mockReturnValue({});
      await expect(service.logout(context)).rejects.toThrow(
