...
export class FriendshipResolver {
  ...
  async create(
    ...
    @CurrentUser() currentUser: CurrentUserI,
  ): Promise<Friendship> {
    return {
      ...(await this.friendshipUseCase.create({
  ...
  async remove(
    @Args('removeFriendshipInput') removeInput: RemoveFriendshipInput,
    @CurrentUser() currentUser: CurrentUserI,
  ) {
    return {
      ...(await this.friendshipUseCase.remove({
        ...removeInput,
        authedUserId: currentUser.sub,
      })),
      currentUserId: currentUser.sub,
    };
  }
  ...
  async findUserFriends(
    @Args('findFriendsByUserInput')
    findFriendsByUserId: FindUserFriendshipsInput,
  ) {
    const a = (
      await this.friendshipUseCase.findUserFriends(findFriendsByUserId)
    ).map((value) => ({
      ...value,
  ...
  async findUsersFriendship(
    @Args('findUsersFriendship')
    findUsersFriendshipInput: FindUsersFriendshipInput,
    @CurrentUser() currentUser: CurrentUserI,
  ) {
    return {
      ...(await this.friendshipUseCase.findUsersFriendship(
        findUsersFriendshipInput,
      )),
      currentUserId: currentUser.sub,
    };
  }
  ...
  async findUserRequests(
    @Args('findFriendsByUserInput')
    findFriendsByUserId: FindUserFriendshipRequestsInput,
    @CurrentUser() currentUser: CurrentUserI,
  ) {
    return (
      await this.friendshipUseCase.findUserRequests({
        userId: currentUser.sub,
  ...
  async count(
    @Args('countFriendshipInput') countInput: CountUserFriendshipsInput,
  ) {
  ...
  anotherUser(@Parent() friendship: Friendship): any {
    const id =
      friendship.currentUserId === friendship.receiverId
        ? friendship.senderId
