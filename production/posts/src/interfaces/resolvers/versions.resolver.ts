...
export class VersionsResolver {
  ...
  @Query(() => [Version])
  findByPost(@Args("findVersionByPostInput") findByPostInput: FindByPostInput): Promise<Version[]> {
    return this.versionsService.findByPost(findByPostInput);
  }

  @Query(() => [Version], { name: "version" })
  findOne(@Args("findVersionInput") findOneVersion: FindOneVersionInput): Promise<Version> {
